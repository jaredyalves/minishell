...
int	is_builtin(t_execute *ecmd)
{
	...
	if (name && *name)
	{
		if (ft_strncmp(name, "echo", ms_strlen(name) + 1) == 0)
			return (1);
		if (ft_strncmp(name, "cd", ms_strlen(name) + 1) == 0)
			return (1);
		if (ft_strncmp(name, "pwd", ms_strlen(name) + 1) == 0)
			return (1);
		if (ft_strncmp(name, "export", ms_strlen(name) + 1) == 0)
			return (1);
		if (ft_strncmp(name, "unset", ms_strlen(name) + 1) == 0)
			return (1);
		if (ft_strncmp(name, "env", ms_strlen(name) + 1) == 0)
			return (1);
		if (ft_strncmp(name, "exit", ms_strlen(name) + 1) == 0)
			return (1);
	}
	return (0);
}
...
void	execute_builtin(t_execute *ecmd)
{
	...
	else if (ft_strncmp(name, "exit", ms_strlen(name) + 1) == 0)
		exit_status = ft_exit(ecmd->argv);
	else if (!has_options(ecmd->argv))
	{
		if (ft_strncmp(name, "cd", ms_strlen(name) + 1) == 0)
			exit_status = ft_cd(ecmd->argv);
		else if (ft_strncmp(name, "pwd", ms_strlen(name) + 1) == 0)
			exit_status = ft_pwd(ecmd->argv);
		else if (ft_strncmp(name, "export", ms_strlen(name) + 1) == 0)
			exit_status = ft_export(ecmd->argv);
		else if (ft_strncmp(name, "unset", ms_strlen(name) + 1) == 0)
			exit_status = ft_unset(ecmd->argv);
		else if (ft_strncmp(name, "env", ms_strlen(name) + 1) == 0)
